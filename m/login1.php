...

{
	...
	if(check_cookies($_COOKIE['QS']['username'],$_COOKIE['QS']['password']))

	{

	update_user_infos($_COOKIE['QS']['uid'],false,true);

			//if($_SESSION['utype']==2)	header("location:personal/user.php");

			if($_SESSION['utype']==2)	header("location:index.php");

			//if($_SESSION['utype']==1)	header("location:company/user.php");

			if($_SESSION['utype']==1)	header("location:index.php");
...
elseif($act == 'waiting_weixin_login'){
	...
	if($uid>0){
...

		{
			...
			setcookie('QS[uid]',$u['uid'],time()+30*24*3600);

			setcookie('QS[username]',$u['username'],time()+30*24*3600);

			setcookie('QS[password]',$u['password'],time()+30*24*3600);

			setcookie('QS[utype]',$u['utype'],time()+30*24*3600);
