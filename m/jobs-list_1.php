...
$_GET['category'] && $postData['category_id'] = intval($_GET['category']);
...
//$postData['district_id'] = intval($_GET['order_type']) ? intval($_GET['order_type']) : 100;
//$postData['business_district_id'] = intval($_GET['order_type']) ? intval($_GET['order_type']) : 100;
$jobList = https_request_api('/job/list', $postData);

if(!$jobList['codes'] && $jobList['data']['list']) {
//    echo '<pre>';
//var_dump($jobList['data']['list']);exit;
}
$smarty->cache = false;
//
...
//	$idresult = $db->query("SELECT id FROM {$jobstable} ".$wheresql.$orderbysql.$limit);//print_r("SELECT id FROM {$jobstable} ".$wheresql.$orderbysql.$limit);exit;
...
//			$jobs[$key]['work_start'] = date("n.j",$value['work_start']);
//			$jobs[$key]['work_end'] = date("n.j",$value['work_end']);
...
//		$jobs=array();
//	}
//	$subsite=$db->getall("select * from ".table("subsite")." where s_effective=1");
	$smarty->assign("subsite",$subsite);
	$smarty->assign('jobs', $jobList['data']['list']);//print_r($jobs);exit;
