...
if($act == 'ajaxjobslist'){
	...
	if (!empty($jobs) && $offset<=1000)
	{
		foreach($jobs as $key=>$li)
		{
			...
			$jobs[$key]['work_start'] = date("n.j",$li['work_start']);
			$jobs[$key]['work_end'] = date("n.j",$li['work_end']);
			$category_cn=explode('/',$list['category_cn']);
			$count=count($category_cn);
			$ke1=$count-1;
			$jobs[$key]['category_cn'] = $category_cn[$ke1];
			if ($li['jobspecial_cn'])
			{
				$jobspecial_cn=explode(',',$li['jobspecial_cn']);
				$jobs[$key]['jobspecial_cn']=$jobspecial_cn;
			}
			else
			{
				$jobs[$key]['jobspecial_cn']=array();
			}
			...
			$jobslisthtml.='<tr><td><a class="jobs-item thisurl box" href="'.$url.'">';
				
											if($key%3==0)
											{
												$jobslisthtml.='<div class="ico ico_n1">'.$li['category_cn'].'</div>';
											}elseif($key%3==1)
											{
												$jobslisthtml.='<div class="ico ico_n2">'.$li['category_cn'].'</div>';
											}else{
												$jobslisthtml.='<div class="ico ico_n3">'.$li['category_cn'].'</div>';
											}
		
					$jobslisthtml.='<div class="txt"><h4>'.$li['jobs_name'].'</h4><h5><em class="n1"></em>'.$jobs[$key]['work_start'].'-'.$jobs[$key]['work_end'].'</h5><h5><em class="n2"></em>'.$li['district_cn'].'</h5></div><div class="sup">';
						if($li['wage_amount']>0)
						{
							$jobslisthtml.='<span>'.$li['wage_amount'].' '.$li['wage_cn'].'</span>';
						}else{
							$jobslisthtml.='<span>'.$li['wage_cn'].'</span>';
						}
						
					$jobslisthtml.='</div><div class="pic"><dl>';
						foreach($jobs[$key]['jobspecial_cn'] as $key1=>$v)
						{
							if($key1%3==0)
							{
								$jobslisthtml.='<dd class="pic1">'.$v.'</dd>';
							}elseif($key1%3==1)
							{
								$jobslisthtml.='<dd class="pic2">'.$v.'</dd>';
							}else{
								$jobslisthtml.='<dd class="pic3">'.$v.'</dd>';
							}
						}
						$jobslisthtml.='</dl></div></a></td></tr>';
			...
			else
			{
				...
				$html='<article class="resumeconlist resumeconlistmb">
						...
						<article class="invitecollectresume flex-box">
							...
							<div class="right flex2"><div id="collect_resume"><p class="fa fa-star-o"></p><p>收藏简历</p></div></div>
						</article>';
			}
		}
...
elseif($act=="ajaxnewjobslist")
{
	...
	$new_jobs = $db->getall("SELECT id,subsite_id,category,category_cn,subclass,jobs_name,district_cn,companyname,work_start,work_end,wage_amount,jobspecial_cn,wage_cn,refreshtime FROM ".table('jobs')." where ".$subsite_wheresql."  ORDER BY `refreshtime` DESC,`id` DESC LIMIT $offset,$rows");
	if (!empty($new_jobs) && $offset<=100)
	{
		foreach($new_jobs as $key=>$list)
		{
			$new_jobs[$key]['url'] = wap_url_rewrite("jobs-show",array("id"=>$list['id']),1,$list['subsite_id']);
			$new_jobs[$key]['r_time'] = daterange(time(),$list['refreshtime'],'Y-m-d',"#FF3300");
			$new_jobs[$key]['work_start'] = date("n.j",$list['work_start']);
			$new_jobs[$key]['work_end'] = date("n.j",$list['work_end']);
			$category_cn=explode('/',$list['category_cn']);
			$count=count($category_cn);
			$ke1=$count-1;
			$new_jobs[$key]['category_cn'] = $category_cn[$ke1];
			if ($list['jobspecial_cn'])
			{
				$jobspecial_cn=explode(',',$list['jobspecial_cn']);
			...
			$newjobshtml.='<tr><td><a class="jobs-item thisurl box" href="'.$new_jobs[$key]['url'].'">';
				
											if($key%3==0)
											{
												$newjobshtml.='<div class="ico ico_n1">'.$list['category_cn'].'</div>';
											}elseif($key%3==1)
											{
												$newjobshtml.='<div class="ico ico_n2">'.$list['category_cn'].'</div>';
											}else{
												$newjobshtml.='<div class="ico ico_n3">'.$list['category_cn'].'</div>';
											}
		
					$newjobshtml.='<div class="txt"><h4>'.$list['jobs_name'].'</h4><h5><em class="n1"></em>'.$new_jobs[$key]['work_start'].'-'.$new_jobs[$key]['work_end'].'</h5><h5><em class="n2"></em>'.$list['district_cn'].'</h5></div><div class="sup">';
						if($list['wage_amount']>0)
						{
							$newjobshtml.='<span>'.$list['wage_amount'].' '.$list['wage_cn'].'</span>';
						}else{
							$newjobshtml.='<span>'.$list['wage_cn'].'</span>';
						}
						
					$newjobshtml.='</div><div class="pic"><dl>';
						foreach($new_jobs[$key]['jobspecial_cn'] as $key1=>$v)
						{
							if($key1%3==0)
							{
								$newjobshtml.='<dd class="pic1">'.$v.'</dd>';
							}elseif($key1%3==1)
							{
								$newjobshtml.='<dd class="pic2">'.$v.'</dd>';
							}else{
								$newjobshtml.='<dd class="pic3">'.$v.'</dd>';
							}
						}
						$newjobshtml.='</dl></div></a></td></tr>';
		}
		exit($newjobshtml);
	}
	else
